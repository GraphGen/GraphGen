...
struct DfsEdge {
  ...
  bool operator<(DfsEdge e) {
    if (this->vertex2 < this->vertex1 && e.vertex2 > e.vertex1) return true;
  ...
  bool operator==(DfsEdge e) {
    if (this->vertex1 != e.vertex1) return false;
    if (this->vertex2 != e.vertex2) return false;
    if (this->label1 != e.label1) return false;
    if (this->edge_label != e.edge_label) return false;
    if (this->label2 != e.label2) return false;
    return true;
  }
...
bool frontEdgeComparator(int i, int j) {
  Edge e1 = universal_edges[i];
  Edge e2 = universal_edges[j];
  if (e1.lbl != e2.lbl) return (e1.lbl < e2.lbl);
  int v1 = (e1.src != universal_u) ? e1.src : e1.dest;
  int v2 = (e2.src != universal_u) ? e2.src : e2.dest;
...
struct Graph {
  ...
  int dfs(int u, int d) {
    ...
    {
      //return is timelimit to calculate min dfs code is exceeeded
      return 0; 
    }
    ...
    for (i = 0; i < back.size(); ++i) {
      ...
      if (code.size() == m) checkForMin(code);
      if (d == 0) return 0;
  ...
  int normal_dfs(int u, int d) {
    ...
    while (i < front.size()) {
      ...
      i++;
      //Since we need to find normal DFS fastly
    }
  ...
  void get_min_dfscode(const char* outfile) {
    ...
    auto start_time_for_loop = chrono::steady_clock::now();
    //start measuring time for making min DFS code
    for (int src = 0; src < n; ++src) {
      
      
      // min dfs code can start only from vertex of least lexographic label
      if (labels[src] != least_label) continue;
      ...
      for (i = 0; i < m; ++i) edge_vis[i] = false;
      dfs_number.resize(n);
      code.clear();
      vertex_code.clear();
      disc = 0;
    ...
    auto lap = chrono::steady_clock::now();
    //Get time to find breaking due to time or due to optimal solution.
    float time_till_now = chrono::duration_cast<chrono::milliseconds>(lap - start_time_for_loop).count();
    // measure time
    ...
    if (!min_code_set)
    {
      cout<<"XTLE: PRINTING SUBOPTIMAL DFS ANSWER |V| = "<<n<<"  |E| = "<<m<<endl;
      //Finding suboptimal DFS code because optimal DFS code wasn't found till time limit
      for (int src = 0; src < n; ++src) {
    ...
    //Print min DFS code in the out file 
    ofstream ofile;
...
int main(int argc, char* argv[]) {
  ...
  //setting global time limit for timeout 
