...
class AddNode(nn.Module):
    ...
    def _initialize_node_repr(self, g, node_type, graph_embed):
        ...
        hv_init = self.initialize_hv(
            torch.cat([
                self.node_type_embed(torch.LongTensor([node_type]).to(device=self.device)),
                graph_embed], dim=1))
...
class ChooseDestAndUpdate(nn.Module):
    ...
    def _initialize_edge_repr(self, g, src_list, dest_list, edge_types):
        ...
        edge_repr = self.edge_type_embed(torch.LongTensor(edge_types).to(device=self.device))
...
class DGMG(nn.Module):
    def __init__(self, v_max, node_hidden_size, num_prop_rounds, 
        dropout_prob, num_node_types, num_edge_types, device=torch.device('cpu')
    ):
...
def create_model(args, feature_map):
    generator = DGMG(v_max=feature_map['max_nodes'], node_hidden_size=args.feat_size,
            num_prop_rounds=args.hops, num_node_types=len(feature_map['node_forward']),
