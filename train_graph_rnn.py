...
def evaluate_loss(args, model, data, feature_map):
    ...
    len_node_vec, len_edge_vec, num_nodes_to_consider = get_attributes_len_for_graph_rnn(
        len(feature_map['node_forward']), len(feature_map['edge_forward']), args.max_prev_node)
    ...
    model['node_level_rnn'].hidden = model['node_level_rnn'].init_hidden(batch_size=batch_size)

    # Teacher forcing: Feed the target as the next input
    # Start token for graph level RNN decoder is node feature second last bit is 1
    node_level_input = torch.cat((torch.zeros(batch_size, 1, x.size(2), device=args.device), x), dim=1)
    node_level_input[:, 0, len_node_vec - 2] = 1
    
    # Forward propogation
    node_level_output = model['node_level_rnn'](node_level_input, input_len=x_len + 1)
    ...
    idx = torch.LongTensor([i for i in range(edge_mat.size(0) - 1, -1, -1)]).to(args.device)
    ...
    edge_mat = edge_mat.reshape(edge_mat.size(0), min(x_len_max - 1, num_nodes_to_consider), len_edge_vec)
    edge_level_input = torch.cat((torch.zeros(sum(x_len), 1, len_edge_vec, device=args.device), edge_mat), dim=1)
    ...
    hidden_edge = model['embedding_node_to_edge'](node_level_output[:, 0:-1, :])
    ...
    idx = torch.LongTensor([i for i in range(hidden_edge.size(0) - 1, -1, -1)]).to(args.device)
    ...
    model['edge_level_rnn'].hidden = torch.cat((hidden_edge, hidden_edge_rem_layers), dim=0)
    
    # Run edge level RNN
    x_pred_edge = model['edge_level_rnn'](edge_level_input, input_len=x_edge_len)
    ...
    x_edge = torch.cat((edge_mat, torch.zeros(sum(x_len), 1, len_edge_vec, device=args.device)), dim=1)
...
def predict_graphs(eval_args):
    ...
    len_node_vec, len_edge_vec, num_nodes_to_consider = get_attributes_len_for_graph_rnn(
        len(feature_map['node_forward']), len(feature_map['edge_forward']), train_args.max_prev_node)
    feature_len = len_node_vec + num_nodes_to_consider * len_edge_vec

    graphs = []

    for _ in range(eval_args.count // eval_args.batch_size):
        model['node_level_rnn'].hidden = model['node_level_rnn'].init_hidden(batch_size=eval_args.batch_size)
        
        # [batch_size] * [num of nodes]
        x_pred_node = np.zeros((eval_args.batch_size, max_num_node), dtype=np.int32)
        # [batch_size] * [num of nodes] * [num_nodes_to_consider]
        x_pred_edge = np.zeros((eval_args.batch_size, max_num_node, num_nodes_to_consider), dtype=np.int32)
    
        node_level_input = torch.zeros(eval_args.batch_size, 1, feature_len, device=eval_args.device)
        ...
        for i in range(max_num_node):
            ...
            node_level_pred = node_level_pred.reshape(eval_args.batch_size, len_node_vec)
            ...
            node_level_input = torch.zeros(eval_args.batch_size, 1, feature_len, device=eval_args.device)
            node_level_input[torch.arange(eval_args.batch_size), 0, sample_node_level_output] = 1 
            ...
            hidden_edge_rem_layers = torch.zeros(
                train_args.num_layers - 1, eval_args.batch_size, hidden_edge.size(2), 
            ...
            edge_level_input = torch.zeros(eval_args.batch_size, 1, len_edge_vec, device=eval_args.device)
            ...
            for j in range(min(num_nodes_to_consider, i)):
                ...
                edge_level_output = edge_level_output.reshape(eval_args.batch_size, len_edge_vec)
        ...
        for k in range(eval_args.batch_size):
            ...
            for v in range(max_num_node):
                ...
                if x_pred_node[k, v] == len_node_vec - 1:
                    ...
                    G.add_node(v, label=feature_map['node_backward'][x_pred_node[k, v]])
                else:
            ...
            for u in range(len(G.nodes())):
                for p in range(min(num_nodes_to_consider, u)):
                    if x_pred_edge[k, u, p] < len(feature_map['edge_forward']):
                        ...
                        G.add_edge(u, v, label=feature_map['edge_backward'][x_pred_edge[k, u, p]])
